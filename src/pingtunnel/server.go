...
import (
	...
	common2 "github.com/mnixry/pingtunnel/src/common"
	frame2 "github.com/mnixry/pingtunnel/src/frame"
	"github.com/mnixry/pingtunnel/src/loggo"
	"github.com/mnixry/pingtunnel/src/threadpool"
...
func (p *Server) processPacket(packet *Packet) {
	...
	if p.maxprocessthread > 0 {
		p.processtp.AddJob((int)(common2.HashString(packet.my.Id)), packet)
	} else {
...
func (p *Server) processDataPacketNewConn(id string, packet *Packet) *ServerConn {
	...
	now := common2.GetNowUpdateInSecond()
	...
	if packet.my.Tcpmode > 0 {
		...
		fm := frame2.NewFrameMgr(FRAME_MAX_SIZE, FRAME_MAX_ID, (int)(packet.my.TcpmodeBuffersize), (int)(packet.my.TcpmodeMaxwin), (int)(packet.my.TcpmodeResendTimems), (int)(packet.my.TcpmodeCompress),
...
func (p *Server) RecvTCP(conn *ServerConn, id string, src *net.IPAddr) {
	...
	startConnectTime := common2.GetNowUpdateInSecond()
	...
	tcpActiveRecvTime := common2.GetNowUpdateInSecond()
	tcpActiveSendTime := common2.GetNowUpdateInSecond()

	for !p.exit && !conn.exit {
		now := common2.GetNowUpdateInSecond()
		sleep := true

		left := common2.MinOfInt(conn.fm.GetSendBufferLeft(), len(bytes))
		if left > 0 {
	...
	startCloseTime := common2.GetNowUpdateInSecond()
	for !p.exit && !conn.exit {
		now := common2.GetNowUpdateInSecond()
