...
import (
	...
	"github.com/mnixry/pingtunnel/src/network"
...
func (p *Client) AcceptTcpConn(conn *net.TCPConn, targetAddr string) {
	...
	uuid := common2.UniqueId()

	fm := frame2.NewFrameMgr(FRAME_MAX_SIZE, FRAME_MAX_ID, p.tcpmode_buffersize, p.tcpmode_maxwin, p.tcpmode_resend_timems, p.tcpmode_compress, p.tcpmode_stat)
	...
	for !p.exit && !clientConn.exit {
		...
		left := common2.MinOfInt(clientConn.fm.GetSendBufferLeft(), len(bytes))
		if left > 0 {
