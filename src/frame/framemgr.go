...
import (
	...
	"github.com/mnixry/pingtunnel/src/congestion"
	"github.com/mnixry/pingtunnel/src/loggo"
	rbuffergo2 "github.com/mnixry/pingtunnel/src/rbuffergo"
	"strconv"
	"sync"
	"time"
)
...
type FrameMgr struct {
	frame_max_size int
	frame_max_id   int32
	sendb          *rbuffergo2.RBuffergo
	recvb          *rbuffergo2.RBuffergo
	...
	sendwin  *rbuffergo2.ROBuffergo
	sendlist *list.List
	sendid   int32

	recvwin  *rbuffergo2.ROBuffergo
	recvlist *list.List
	recvid   int32
...
func NewFrameMgr(frame_max_size int, frame_max_id int, buffersize int, windowsize int, resend_timems int, compress int, openstat int) *FrameMgr {
	...
	sendb := rbuffergo2.New(buffersize, false)
	recvb := rbuffergo2.New(buffersize, false)
	...
	fm := &FrameMgr{
		...
		sendwin:  rbuffergo2.NewROBuffer(windowsize, 0, frame_max_id),
		sendlist: list.New(), sendid: 0,
		recvwin:  rbuffergo2.NewROBuffer(windowsize, 0, frame_max_id),
...
func (fm *FrameMgr) cutSendBufferToWindow(cur int64) {
	...
	for fm.sendb.Size() >= fm.frame_max_size && fm.sendwin.Size() < int(fm.windowsize) {
		...
		if fm.compress > 0 && len(fd.Data) > fm.compress {
			newb := common2.CompressData(fd.Data)
...
func (fm *FrameMgr) processRecvList(tmpreq map[int32]int, tmpack map[int32]int, tmpackto map[int32]*Frame) {
	...
	if len(tmpackto) > 0 {
		tmpsize := common2.MinOfInt(len(tmpackto), fm.frame_max_size/2/4)
...
func (fm *FrameMgr) processRecvFrame(f *Frame) bool {
	if f.Data.Type == (int32)(FrameData_USER_DATA) {
		...
		if left >= len(f.Data.Data) {
			src := f.Data.Data
			if f.Data.Compress {
				old, err := common2.DeCompressData(src)
				if err != nil {
